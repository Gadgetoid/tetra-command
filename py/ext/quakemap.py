"""A world map showing recent notable quakes."""
...
SCALE_NEAR = 1.9 * look.SCALE
SCALE_FAR = 1.05 * look.SCALE
SCALE_KNEE = 10.0
SCALE_RATE = 0.4
EASE_MS = 420
...
def _travel(view, active, elapsed):
    """Ease the view toward the active event, with the zoom set by the distance."""
...
def _band(theme, event, index, total, note="waiting for the feed"):
    ...
    draw.blit_label(magnitude, look.SIZE_BIG,
                    draw.readable(theme.at(_mag_fraction(event["mag"])), theme.panel,
                                  theme.ink),
                    look.PAD, BAND_TOP + look.px(3))
    left = look.PAD + draw.text_width(magnitude, look.SIZE_BIG) + look.px(10)

    where = f"{index + 1}/{total}"
    draw.blit_label(where, look.SIZE_SMALL, theme.dim, look.W - look.PAD, BAND_TOP + look.px(4),
                    align=2)
