"""The ISS: the station, its ground track, and the terminator, over a world map."""
...
def eased(held, target):
    """Move `held` a share of the way to `target`, the short way round the date line."""
    if held is None:
        return target
...
def _marker(theme, view, lon, lat, sunlit):
    """Draw the station: lit in sunlight, quiet in shadow, over a soft halo."""
    x, y = view.at(lon, lat)
...
def _smoothed(points):
    """Resample the ground track to a curve through every point the host sent."""
    lons, lats, lit = [], [], []
    turns = 0.0
...
def _curve(state, points):
    """Rebuild the smoothed track when the host sends a new run."""
...
def _track(theme, view, state, points, flown):
    """Draw the ground track, split at `flown`."""
    if len(points) < 2:
        return
    _curve(state, points)
    dense = state["curve"]
...
def _band(theme, where, aboard, at=None, note="waiting for the feed"):
    """Draw the strip under the map: how high, how fast, in sun or shadow, and who is aboard."""
    screen.pen = theme.panel
    screen.rectangle(rect(0, BAND_TOP, look.W, BAND_H))
    screen.pen = theme.accent_b
    screen.rectangle(rect(0, BAND_TOP, look.W, max(1, look.px(1))))
    if not where:
        if note:
            draw.blit_label(note, look.SIZE_VALUE, theme.dim, look.PAD, BAND_TOP + look.px(9))
        return

    unit = where.get("unit") or "km"
    altitude = f"{where['altitude']:.0f} {unit}" if where.get("altitude") else "--"
    draw.blit_label(altitude, look.SIZE_BIG, theme.ink, look.PAD, BAND_TOP + look.px(3))
    left = look.PAD + draw.text_width(altitude, look.SIZE_BIG) + look.px(10)
    ...
    draw.blit_label(lit, look.SIZE_SMALL, pen, look.W - look.PAD, BAND_TOP + look.px(4), align=2)

    draw.blit_label(_where_text(at[1], at[0]) if at else _where_text(None, None),
                    look.SIZE_LABEL, theme.ink, left, BAND_TOP + look.px(3))
    ...
    if detail:
        draw.blit_label(", ".join(detail), look.SIZE_SMALL, theme.dim, left, BAND_TOP + look.px(19))
...
def _where_text(lat, lon):
    """Format a latitude and longitude as "51.6N 30.2E"."""
...
def render(page, frame, _history, theme):
    ...
    if not worldmap.ready():
        draw.blit_label("loading the map", look.SIZE_VALUE, theme.dim,
                        look.W // 2, MAP_TOP + MAP_H // 2 - look.px(8), align=1)
